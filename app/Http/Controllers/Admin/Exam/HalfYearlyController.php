...
class HalfYearlyController extends Controller
{
    public function update_hy_xi(Request $request, $id) {
        ...
        $isInfo = DB::table('half_yearly_exam')->where('c_class', 'XI')->where('st_id', $id)->first();

        if($isInfo != null) {
            DB::table('half_yearly_exam')->where('c_class', 'XI')->where('st_id', $id)->update($data);
        }
        else {
            $data['c_class'] = 'XI';
            $data['st_id'] = $id;
